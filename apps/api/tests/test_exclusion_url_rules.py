...
def test_path_prefix_excluded() -> None:
    ...
    excluded, reason, page_type, _ = should_exclude("https://example.com/get-quote")
...
def test_path_prefix_quote() -> None:
    excluded, reason, _, _ = should_exclude("https://example.com/quote/start")
    assert excluded is True
...
def test_path_contains_step_excluded() -> None:
    ...
    excluded, reason, page_type, _ = should_exclude("https://example.com/about/step-1")
    assert excluded is True
...
def test_query_key_excluded() -> None:
    ...
    excluded, reason, _, _ = should_exclude("https://example.com/page?session=abc")
    assert excluded is True
...
def test_query_key_step_excluded() -> None:
    ...
    excluded, reason, _, _ = should_exclude("https://quote.unitedglobalvanline.com/estimate?step=1")
    assert excluded is True
    assert "step" in reason or "estimate" in reason
    excluded2, reason2, _, _ = should_exclude("https://example.com/page?Step=2")
...
def test_query_prefix_utm_excluded() -> None:
    ...
    excluded, reason, _, _ = should_exclude("https://example.com/page?utm_source=google")
    assert excluded is True
...
def test_allowed_url() -> None:
    ...
    excluded, reason, page_type, _ = should_exclude("https://example.com/about")
...
def test_html_text_unused() -> None:
    ...
    excluded1, _, _, _ = should_exclude("https://example.com/about")
    excluded2, _, _, _ = should_exclude("https://example.com/about", html="<html></html>", text="Hello")
