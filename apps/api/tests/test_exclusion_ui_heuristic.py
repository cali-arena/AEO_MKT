...
def test_ui_flow_heuristic_excludes_form_heavy_short_text() -> None:
    ...
    excluded, reason, details = ui_flow_heuristic(FORM_HEAVY_HTML, SHORT_TEXT)
    assert excluded is True
    assert "ui_form_heuristic" in reason
    assert "text_len=" in reason
    assert "tag_hits=" in reason
    assert "density=" in reason
    assert details["text_len"] == len(SHORT_TEXT.strip())
    assert details["tag_hits"] >= 1
...
def test_ui_flow_heuristic_allows_long_text() -> None:
    ...
    excluded, reason, details = ui_flow_heuristic(FORM_HEAVY_HTML, long_text)
...
def test_should_exclude_calls_heuristic_when_html_text_provided() -> None:
    ...
    excluded, reason, page_type, heuristic_info = should_exclude(
    ...
    assert heuristic_info is not None
    assert "tag_hits" in heuristic_info
...
def test_should_exclude_skips_heuristic_without_html_text() -> None:
    ...
    excluded, _, _, heuristic_info = should_exclude("https://example.com/info")
    assert excluded is False
    assert heuristic_info is None
